...
class Swap extends CI_Controller
{
    ...
    function swapping()
    {
    	...
    	$p="SELECT * FROM ".get_school_db().".swap sw ". $teacher_query." WHERE sw.school_id=".$_SESSION['school_id']." ". $date_query."  ORDER BY sw.swap_date desc";
    	
    	$config['base_url'] = base_url()."swap/swapping/".$teacher_id."/".$start_date."/".$end_date;

        $page_data['apply_filter']=$apply_filter;
    	$page_data['swap_quer']= $record_count;  // $swap_quer;
        $page_data['start_date']=$start_date;
        $page_data['end_date']=$end_date;
        $page_data['teacher_id']=$teacher_id;
        $page_data['page_title'] = get_phrase('add_teacher_swap');
        $page_data['page_name']  = 'manage_teacher_swap';
        $this->session->set_flashdata('club_updated', get_phrase('teacher_swapping_successfully.'));
        $this->load->view('backend/index', $page_data);
    }
