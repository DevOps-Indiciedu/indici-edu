...
<div class="tab-content tcher-dboard"> 
        ...
        <div class="row">
            <div class="col-sm-12 col-md-12 b-s p-4">
                <div class="box">
                    ...
                    <div id="text"> 
                            <?php
                            if (count($today_subjects_arr) > 0)
                            {  
                                $diary_index = 0;
                                foreach ($today_subjects_arr as $diary_outer)
                                {
                                    $class_section_name = $today_subjects_section_arr[$diary_index]; 
                                    ++$diary_index;
                            ?>
                                    <div class="row">
                                        <div class="col-md-12 col-sm-12">
                                            <h3 style="font-size:18px;">
                                                <?php echo $class_section_name . '  /   ' . get_subject_name($diary_outer); ?>
                                            </h3> 
                                        </div>
                                    ...
                                    $diary = $this->db->query("select dr.*,  concat(sub.name,' - ', sub.code) as subject_name
                                                               ...
                                                               AND dr.subject_id = $diary_outer ")->result_array();
                                    if (count($diary) > 0)
                                    {
                                        foreach($diary as $dr)
                                        { ?>
                                    ...
                                    {
                                        echo '<span class="text-danger" style="padding-left:2%">'.get_phrase('no_diary_added').'</span>';
                                    }
                                    ?>
                                    </div>
                                <?php    
    ...
    <div id="attendance" class="tab-pane dash-m">
        ...
        $qur_red = $this->db->query("select a.status,a.date FROM ".get_school_db().".attendance_staff a
            ...
            <table cellpadding="0" cellspacing="0" border="0" class="table table-bordered table-responsive" aria-describedby="table_export_info">
                ...
                <tbody>
                    ...
                    for($i=01; $i<=$d; $i++)
                    {
                        ...
                        echo "<td><span style='!important;'>$d1</span> </td>";
                           echo "<td>$dw</td>";
                        echo "<td>";
                        if(is_array($plan[$today_date]) || is_object($plan[$today_date])){
                            foreach($plan[$today_date] as $std )
                            {
                                if($std['status']==1)
                                {
                                    echo "<span class='prsnt'>".get_phrase('present')."</span>";
                                }
                                elseif($std['status']==2)
                                {
                        ...
                        if(is_array($plan[$today_date])|| is_object($plan[$today_date])){
                            foreach($plan[$today_date] as $std )
                            {
                                ...
                                if($std['status']==3)
                                {
                                    ...
                                    echo "<strong>".get_phrase('category').":</strong>".$this->Crud_model->get_type_name_by_id('leave_category',$leave_arr[0]['leave_category_id'],'');
            ...
            <div class="col-sm-12 col-md-12 b-s p-4">
                <div class="box">
                    ...
                    <div style="/*overflow:hidden;   height:263px;*/" id="text">
                        ...
                        foreach ($today_subjects_arr as $planner_outer) 
                        {
                            $class_section_name = $today_subjects_section_arr[$acc_planner_dashboard_counter];
                            ++$acc_planner_dashboard_counter;
                        ?>
                            <div class="row">
                                <div class="col-md-12 col-sm-12">
                                    <h3 style="font-size:18px;color:black !important">
                                        <?php echo $class_section_name . ' /  ' . get_subject_name($planner_outer); ?> 
                                    </h3> 
                                </div>
                            <?php
                            $planner_arr =$this->db->query("select p.*
                                ...
                                and (DATE(NOW()) = p.start)
                                and p.subject_id in ($planner_outer) ")->result_array();
    ...
    <li role="presentation"><a href="#detail<?php echo $pr['planner_id']; ?>_<?php echo $acc_planner_dashboard_counter ?>" aria-controls="settings" role="tab" data-toggle="tab"><?php echo get_phrase('detail');?></a></li>
    ...
    <li role="presentation" class="active"><a href="#assessment<?php echo $pr['planner_id']; ?>_<?php echo $acc_planner_dashboard_counter ?>" aria-controls="home" role="tab" data-toggle="tab"><?php echo get_phrase('assesment');?></a></li>
  ...
  <div role="tabpanel" class="tab-pane" id="detail<?php echo $pr['planner_id']; ?>_<?php echo $acc_planner_dashboard_counter ?>"><h3>
    ...
    <div role="tabpanel" class="tab-pane active" id="assessment<?php echo $pr['planner_id']; ?>_<?php echo $acc_planner_dashboard_counter ?>"><h3>
    ...
    <div role="tabpanel" class="tab-pane" id="requirement<?php echo $pr['planner_id']; ?>_<?php echo $acc_planner_dashboard_counter ?>"><h3>
                            ...
                            {
                            ?>
                               <p class="text-danger" style="padding-left:2%">No Academic Planner Found For Today</p>
                            <?php
                            }
                            ?>
                            </div>
